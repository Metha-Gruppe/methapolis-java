...
public class ResearchState extends JFrame implements ActionListener, Serializable {
	...
	/*
	 * // CONSTRUCTOR public ResearchState(Micropolis engine, ResearchTree tree)
	 * { city = engine; this.tree = tree; reached_nodes = new
	 * HashSet<Integer>();
	 * 
	 * setName("Research Tree"); setLayout(new BorderLayout());
	 * 
	 * insidePane = new JPanel(); insidePane.setLayout(null);
	 * 
	 * scrollPane = new JScrollPane(insidePane); // scrollPane.setLayout(null);
	 * add(scrollPane, BorderLayout.CENTER);
	 * 
	 * subMenu = new JPanel(); subMenu.setLayout(new BorderLayout());
	 * add(subMenu, BorderLayout.SOUTH);
	 * 
	 * researchPointsLabel = new JLabel(Integer.toString(researchPoints) +
	 * " research points"); subMenu.add(researchPointsLabel, BorderLayout.EAST);
	 * 
	 * JPanel ppanel = new JPanel(); ppanel.setLayout(new
	 * FlowLayout(FlowLayout.CENTER)); subMenu.add(ppanel, BorderLayout.CENTER);
	 * 
	 * closeButton = new JButton("close"); closeButton.addActionListener(new
	 * ActionListener() {
	 * 
	 * @Override public void actionPerformed(ActionEvent arg0) {
	 * setVisible(false); } }); ppanel.add(closeButton);
	 * 
	 * int n = ResearchTree.possible_nodes.length; buttons = new JButton[n];
	 * 
	 * for (int node_id = 0; node_id < n; node_id++) { ResearchNode node =
	 * ResearchTree.possible_nodes[node_id];
	 * 
	 * buttons[node_id] = new JButton("<html>" + node.getDesc() + "</html>");
	 * buttons[node_id].setBounds(new
	 * Rectangle(ResearchTree.positionsX[node_id],
	 * ResearchTree.positionsY[node_id], buttonWidth, buttonHeight));
	 * 
	 * buttons[node_id].setActionCommand(Integer.toString(node_id));
	 * buttons[node_id].addActionListener(this);
	 * 
	 * maxWidth = Math.max(maxWidth, ResearchTree.positionsX[node_id] +
	 * buttonWidth); maxHeight = Math.max(maxHeight,
	...
	private void refreshNodesFromResearchData() {
		...
		int[] progress = {
				researchData.rocketResearch, researchData.policeResearch, researchData.fireResearch,
				researchData.environmentResearch
		};
	...
	public void refreshPanel() {
		...
		for(int node_id = 0; node_id < ResearchTree.possible_nodes.length; node_id++) {
			...
			// reachable
				...
				buttons[node_id].setText("<html>" + node.getName() + " (" + Integer.toString(node.getCost()) + "pts)"
						+ node.getDesc() + "</html>");
			}
		}
